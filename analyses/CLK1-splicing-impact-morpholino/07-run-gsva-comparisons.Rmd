...
top_anno <- HeatmapAnnotation(
  ...
  annotation_legend_param = list(
    Treatment = list(
      title = "Treatment",
      at = c("Non-targeting Morpholino", "CLK1 Exon 4 Morpholino"),
      labels = c("Non-targeting Morpholino", expression(italic("CLK1")~"Exon 4 Morpholino"))
    )
  )
)
...
for (i in 1:length(score_sources)) {
  ...
  score_df_summarized <- score_df %>%
    ...
    group_by(geneset) %>%
  pivot_wider(names_from = Treatment, values_from = Mean_GSVA_Score) %>%
  # keep rows that have both treatment means
  filter(!is.na(`CLK1 Exon 4 Morpholino`), !is.na(`Non-targeting Morpholino`)) %>%
  mutate(
    GSVA_diff = `CLK1 Exon 4 Morpholino` - `Non-targeting Morpholino`,
    diff_sign = ifelse(GSVA_diff > 0, "Up in CLK1 Exon 4 Morpholino", "Down in CLK1 Exon 4 Morpholino")  # new column for coloring
  )
...
score_df_summarized$geneset <- factor(score_df_summarized$geneset, levels = ordering_metric)
  ...
  gsva_barplot <- ggplot(score_df_summarized, aes(x = geneset, y = GSVA_diff, fill = diff_sign)) +
  ...
  labs(x = "Geneset", 
       ...
       fill = "Direction") +
  scale_fill_manual(
    breaks = c("Down in CLK1 Exon 4 Morpholino", "Up in CLK1 Exon 4 Morpholino"),
    values = c(
      "Down in CLK1 Exon 4 Morpholino" = "blue",
      "Up in CLK1 Exon 4 Morpholino"   = "darkorange"
    ),
    labels = c(
      expression("Down in"~italic("CLK1")~"Exon 4 Morpholino"),
      expression("Up in"~italic("CLK1")~"Exon 4 Morpholino")
    )
  ) +
  coord_flip() +
