...
crispr_file <- file.path(input_dir,"CRISPRGeneEffect.csv")
crispr_df <- vroom(file.path(crispr_file))
kinase_list <- vroom(file.path(root_dir,"analyses/splicing_events_functional_sites/results/kinases-functional_sites.tsv")) %>%
  group_by(gene) %>%
  filter(Frequency == max(Frequency, dPSI)) %>%
  select(gene, Frequency, dPSI) %>%
  unique()

crispr_df_long <- crispr_df %>%
  filter(...1=="ACH-000622") %>%  # Filter for a specific row
...
crispr_kinase_df <- inner_join(crispr_df_long,kinase_list, by='gene') %>%
  select(gene,CRISPR_score,Frequency,dPSI) 

crispr_kinase_df <- crispr_kinase_df %>%
  arrange(CRISPR_score) %>%
  mutate(gene = factor(gene, levels = unique(gene))) %>%
  filter(Frequency > 10)

kinase_crispr_plot <- ggplot(data = crispr_kinase_df, aes(x = gene, y = CRISPR_score, size = dPSI)) +
  # All points with solid grey color
  geom_point(colour = "gray50", fill = "gray50", pch = 21, size = 3) +  # Solid grey circles
  
  # Highlight CLK1 with red circles
  geom_point(data = subset(crispr_kinase_df, gene == "CLK1"), 
             colour = "red", fill = "red", size = 3) +  # Red circle for CLK1
  
  xlab("Kinase") + 
  ylab("CRISPR Dependency Score") + 
  ggtitle("Perturbations in KNS42") +
  theme_Publication() + 
  theme(axis.text.x = element_text(angle = 75, hjust = 1, size = 8)) +
  scale_size_continuous(range = c(3, 10))  # Adjust size range if needed

# Display the plot
file_depmap_cns_score_kinase_plot = file.path(plots_dir,"depmap_score_all_kinases_kns42lines.pdf")
pdf(file_depmap_cns_score_kinase_plot, height = 4, width = 4)
print(kinase_crispr_plot)
dev.off()
