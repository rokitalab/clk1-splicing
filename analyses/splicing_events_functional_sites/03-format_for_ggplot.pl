...
open(my $out_pos, '>', 'results/splicing_events.SE.total.PS.pos.intersectunip.ggplot.txt') or die "Cannot open file: $!";
print $out_pos "SpliceID\tdPSI\tUniprot\n";

open(my $out_neg, '>', 'results/splicing_events.SE.total.PS.neg.intersectunip.ggplot.txt') or die "Cannot open file: $!";
print $out_neg "SpliceID\tdPSI\tUniprot\n";

sub process_file {
    my ($prefix, $suffix, $category, $output) = @_;

    open(my $in, '<', "results/splicing_events.SE.total.PS.$prefix.intersectunip$suffix.hg38.col.wo.txt") or die "Cannot open file: $!";
    my %seen;
    while (my $line = <$in>) {
        chomp $line;
