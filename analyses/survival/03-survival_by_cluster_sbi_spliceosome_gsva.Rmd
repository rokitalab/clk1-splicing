...
```{r}
# Generate kaplan meier survival models for OS and EFS, and save outputs
c8_si_kap_os <- survival_analysis(
  metadata  = cluster8_df %>% dplyr::filter(!is.na(SI_group)),
  ind_var = "SI_group",
  test = "kap.meier",
  metadata_sample_col = "Kids_First_Biospecimen_ID",
  days_col = "OS_days",
...
readr::write_rds(c8_si_kap_os,
                 file.path(results_dir, "logrank_cluster8_OS_SBI.RDS"))

c8_si_kap_efs <- survival_analysis(
  metadata  = cluster8_df %>% dplyr::filter(!is.na(SI_group)),
  ind_var = "SI_group",
  test = "kap.meier",
  metadata_sample_col = "Kids_First_Biospecimen_ID",
...
readr::write_rds(c8_si_kap_efs,
                 file.path(results_dir, "logrank_cluster8_EFS_SBI.RDS"))
```

Generate Cluster 8 KM SI_group plots

```{r}
km_c8_si_os_plot <- plotKM(model = c8_si_kap_os,
                    variable = "SI_group",
                    combined = F, 
                    title = "Cluster 8, overall survival",
                    p_pos = "topright")

ggsave(file.path(plot_dir, "km_cluster8_OS_sbi_group.pdf"),
       km_c8_si_os_plot,
       width = 8, height = 5, units = "in",
       device = "pdf")

km_c8_si_efs_plot <- plotKM(model = c8_si_kap_efs,
                    variable = "SI_group",
                    combined = F, 
                    title = "Cluster 8, event-free survival",
                    p_pos = "topright")

ggsave(file.path(plot_dir, "km_cluster8_EFS_sbi_group.pdf"), 
       km_c8_si_efs_plot,
       width = 8, height = 5, units = "in",
       device = "pdf")
```
...
```{r}
# Generate kaplan meier survival models for OS and EFS, and save outputs
c8_splice_kap_os <- survival_analysis(
...
readr::write_rds(c8_splice_kap_os,
                 file.path(results_dir, "logrank_cluster8_OS_splice_group.RDS"))

c8_splice_kap_efs <- survival_analysis(
  metadata  = cluster8_df %>% 
    dplyr::filter(spliceosome_group %in% c("Splice GSVA 4th Q", "Splice GSVA 1st Q")) %>%
    dplyr::mutate(spliceosome_group = factor(spliceosome_group,
                                                  levels = c("Splice GSVA 1st Q", "Splice GSVA 4th Q"))),
...
readr::write_rds(c8_splice_kap_efs,
                 file.path(results_dir, "logrank_cluster8_EFS_splice_group.RDS"))
```

Generate Cluster 8 KM spliceosome_group plots

```{r}
km_c8_splice_os_plot <- plotKM(model = c8_splice_kap_os,
                    variable = "spliceosome_group",
                    combined = F, 
                    title = "Cluster 8, overall survival",
                    p_pos = "topright")

ggsave(file.path(plot_dir, "km_cluster8_OS_splice_group.pdf"),
       km_c8_splice_os_plot,
       width = 9, height = 5, units = "in",
       device = "pdf")

km_c8_splice_efs_plot <- plotKM(model = c8_splice_kap_efs,
                    variable = "spliceosome_group",
                    combined = F, 
                    title = "Cluster 8, event-free survival",
                    p_pos = "topright")

ggsave(file.path(plot_dir, "km_cluster8_EFS_splice_group.pdf"), 
       km_c8_splice_efs_plot,
       width = 9, height = 5, units = "in",
       device = "pdf")
```
...
add_model_c8_efs <- fit_save_model(cluster8_df %>% 
                                      ...
                                      dplyr::mutate(plot_group = fct_relevel(plot_group,
                                                                            c("Other high-grade glioma", "Atypical Teratoid Rhabdoid Tumor",
                                                                              "Diffuse midline glioma", "Ependymoma", "Mesenchymal tumor",
...
forest_c8_spliceosome_efs <- plotForest(readRDS(file.path(results_dir, "cox_hgg_EFS_additive_terms_subtype_cluster_spliceosome_score.RDS")))

ggsave(file.path(plot_dir, "forest_add_EFS_cluster8_histology_resection_spliceosome_group.pdf"),
       forest_c8_spliceosome_efs,
...
forest_c8_si_os <- plotForest(readRDS(file.path(results_dir, "cox_hgg_OS_additive_terms_subtype_cluster_si_group.RDS")))

ggsave(file.path(plot_dir, "forest_add_OS_cluster8_histology_resection_si.pdf"),
       forest_c8_si_os,
       width = 9, height = 4, units = "in",
       device = "pdf")
```
