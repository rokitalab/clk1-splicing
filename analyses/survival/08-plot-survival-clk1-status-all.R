...
for (group in groups){
  ...
  if (grepl("GNG|LGG|HGG|MB", group)){
    ...
    os_survival_result <- read_rds(
      file.path(input_dir,
                glue::glue("cox_{file_names[group]}_OS_additive_terms_subtype_clk1_group.RDS")
      ))
  }
  ...
  if (grepl("GNG|LGG|HGG|MB", group)){
    ...
    efs_survival_result <- read_rds(
      file.path(input_dir,
                glue::glue("cox_{file_names[group]}_EFS_additive_terms_subtype_clk1_group.RDS")
      ))
  }
