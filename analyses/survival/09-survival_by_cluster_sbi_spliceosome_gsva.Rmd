...
int_model_efs <- fit_save_model(metadata[!metadata$extent_of_tumor_resection %in% c("Not Reported", "Unavailable"),],
                              terms = "extent_of_tumor_resection+cluster*spliceosome_gsva_score+age_at_diagnosis_days",
                               file.path(results_dir, "cox_EFS_interaction_terms_resection_lgg_group_cluster_spliceosome_score.RDS"),
                               "multivariate",
                               years_col = "EFS_years",
                               status_col = "EFS_status")

int_forest_efs <- plotForest(readRDS(file.path(results_dir, "cox_EFS_interaction_terms_resection_lgg_group_cluster_spliceosome_score.RDS")))

int_forest_efs

ggsave(file.path(plot_dir, "forest_int_EFS_resection_lgg_group_cluster_assignment_spliceosome_score.pdf"),
       int_forest_efs,
       width = 10, height = 6, units = "in",
       device = "pdf")
...
cluster6_df <- metadata %>%
  ...
  dplyr::mutate(spliceosome_group = case_when(
      spliceosome_gsva_score > summary(spliceosome_gsva_score)["3rd Qu."] ~ "High Spliceosome Score",
      spliceosome_gsva_score < summary(spliceosome_gsva_score)["1st Qu."] ~ "Low Spliceosome Score",
      TRUE ~ NA_character_
    )) %>%
  dplyr::mutate(SI_group = fct_relevel(SI_group,
                                                 c("Low SBI", "High SBI"))) %>%
  dplyr::mutate(spliceosome_group = fct_relevel(spliceosome_group,
                                                 c("Low Spliceosome Score", 
                                                   "High Spliceosome Score")))
```
...
km_hgg_os_plot <- plotKM(model = hgg_kap_os,
                    variable = "SI_group",
                    combined = F, 
                    title = "Splicing Burden Index group")
...
# Generate kaplan meier survival models for OS and EFS, and save outputs
hgg_kap_os <- survival_analysis(
  metadata  = cluster6_df %>% dplyr::filter(!is.na(spliceosome_group)),
...
km_hgg_os_plot <- plotKM(model = hgg_kap_os,
                    ...
                    combined = F, 
                    title = "Spliceosome GSVA score group")
