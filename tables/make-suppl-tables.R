...
kegg_splice <- file.path(analysis_dir, "sample-psi-clustering", "results", "kegg_spliceosome_summary.tsv")
...
## sheet 4, kegg spliceosome scores
kegg_splice_df <- read_tsv(kegg_splice) %>%
  dplyr::mutate(Filename = gsub("_pathway_stranded.tsv", "", Filename)) %>%
